...
$nido_id = apply_filters( 'nido_get_nido_id', $user->ID );

?>
<div class="nido-password-header">
    <img src="<?php echo esc_url( apply_filters( 'nido_get_avatar', $nido_id ) ) ?>">
    <label><?php esc_html_e( apply_filters( 'nido_get_name', $nido_id ) ) ?></label>
</div>
<div class="tml tml-profile nido-password-page" id="theme-my-login<?php $template->the_instance(); ?>">
	<?php $template->the_action_template_message( 'profile' ); ?>
    <?php 
        $updated = isset( $_GET['updated'] ) ? $_GET['updated'] : '';
        $back = isset( $_GET['back'] ) ? urldecode( $_GET['back'] ) : '';
        ...
        if ( $updated != '' && $back != '' ) {
            ...
            <script type="text/javascript">
                jQuery.confirm( {
                    ...
                    content: 'La contraseña ha sido actualizada. Será redirigido en unos segundos.',
                    type: 'green',
                    theme: 'nido',
                    typeAnimated: true,
                    buttons: {
	...
	<form id="your-profile" action="<?php $template->the_action_url( 'profile', 'login_post' ); ?>" method="post">
		...
		<h3 class="nido-hide"><?php _e( 'Personal Options', 'theme-my-login' ); ?></h3>

		<table class="tml-form-table nido-hide">
		...
		<h3 class="nido-hide"><?php _e( 'Name', 'theme-my-login' ); ?></h3>
		...
		<h3 class="nido-hide"><?php _e( 'Contact Info', 'theme-my-login' ); ?></h3>
		...
		<h3 class="nido-hide"><?php _e( 'About Yourself', 'theme-my-login' ); ?></h3>
		...
		<h3 class="nido-hide"><?php _e( 'Account Management', 'theme-my-login' ); ?></h3>
		<table class="tml-form-table">
		<tr id="password" class="user-pass1-wrap">
			<th style="border: none"><label for="pass1"><?php _e( 'New Password', 'theme-my-login' ); ?></label></th>
			<td>
